...
{
    [SerializeField] private Transform _targetTransform; // es el enemigo
    [SerializeField] private Transform _cannonPivot; // punto del cañon
    [SerializeField] private Transform _shipReferenceTransform; // es el barco 

    [SerializeField] private float _yawLimit = 145f; // limites del giro
    [SerializeField] private float _projectileSpeed = 250f; // velocidad de la bala
    [SerializeField] private Vector2 _pitchLimits = new Vector2(0f, 45f); //limites para qu emire arriba y abajo

    private float _startingYaw; //la posicion en la que inicia la torreta

    private void Awake()
    {
        _startingYaw = Mathf.DeltaAngle(0f, transform.localEulerAngles.z); // lo pone en la posicion inicial
    }
    ...
    private void RotateTurretBase()
    {
        ...
        Vector3 localDirectionToTarget = _shipReferenceTransform.InverseTransformDirection(directionToTarget); // convierte de coordenadas globales a coordenadas locales 
        ...
        transform.localRotation = Quaternion.Euler(0f, 0f, _startingYaw + limitedYawDifference); //calcula la direccion en la que esta el enemigo y gira hacia ella
    }
    ...
    private void ElevateCannon()
    {
        ...
        _cannonPivot.localRotation = Quaternion.Euler(0f, limitedCannonPitchAngle, 0f); // calcula la elevacion necesaria para que le dispare al enemigo usando el private de abajo
    }
    ...
    private bool TryCalculateCannonPitchAngle(out float cannonPitchAngle)
    {
        ...
        return true; // calcula el angulo necesario con el cual la bala saldria para que le de al enemigo asi como su fuerza necesaria y para que haga la parabola del disparo
    }
}