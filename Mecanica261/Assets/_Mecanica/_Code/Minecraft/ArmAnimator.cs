...
public class ArmAnimator : MonoBehaviour
{
    ...
    private void Awake()
    {
        _startRotations = new Quaternion[_armPivots.Length];

        for (int i = 0; i < _armPivots.Length; i++)
        {
            if (_armPivots[i] == null) { continue; }

            _startRotations[i] = _armPivots[i].localRotation;
        }
    ...
    private void AnimateMovement(float horizontalSpeed)
    {
        ...
        float swing = Mathf.Sin(_animationTime);

        for (int i = 0; i < _armPivots.Length; i++)
        {
            float direction = i % 2 == 0 ? 1f : -1f;
            ApplyArmRotation(i, swingRotation * swing * direction);
        }
    }

    private void ApplyArmRotation(int index, Vector3 rotation)
    {
        if (_armPivots[index] == null) { return; }

        Quaternion targetRotation = _startRotations[index] * Quaternion.Euler(rotation);
        _armPivots[index].localRotation = Quaternion.Lerp(_armPivots[index].localRotation, targetRotation, _rotationSmoothTime * Time.deltaTime);
    }

    private Vector3 MirrorRotation(Vector3 rotation, int index)
    {
        float direction = index % 2 == 0 ? 1f : -1f;

        return new Vector3(rotation.x, rotation.y * direction, rotation.z * direction);
    }
}
