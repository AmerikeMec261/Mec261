...
namespace Minecraft
{
    public class LegAnimator : MonoBehaviour
    {
        ...
        private void Awake()
        {
            _startingRotations = new Quaternion[_legPivots.Length];

            for (int i = 0; i < _legPivots.Length; i++)
            {
                _startingRotations[i] = _legPivots[i].localRotation;
            }
        ...
        private void MoveLegs(float horizontalSpeed)
        {
            ...
            float stepRotationAmount = Mathf.Sin(_stepTime) * stepAngle;

            for (int i = 0; i < _legPivots.Length; i++)
            {
                float stepDirection = i % 2 == 0 ? 1f : -1f;
                Quaternion stepRotation = Quaternion.AngleAxis(stepRotationAmount * stepDirection, _rotationAxis);
                _legPivots[i].localRotation = _startingRotations[i] * stepRotation;
            }
        }
        ...
        private void ReturnToIdle()
        {
            for (int i = 0; i < _legPivots.Length; i++)
            {
                _legPivots[i].localRotation = Quaternion.Lerp(_legPivots[i].localRotation, _startingRotations[i], _idleReturnSpeed * Time.deltaTime);
            }
        }
    }
}
